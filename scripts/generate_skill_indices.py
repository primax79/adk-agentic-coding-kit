...
1. Plugin Level (e.g. plugins/adk-tools/index.json)
2. Skills Directory Level (e.g. plugins/adk-tools/skills/index.json)
3. Individual Skill Level (e.g. plugins/adk-tools/skills/adk-agent-architecture/index.json)
    ...
    for rel_plugin_dir, skills_sub in PLUGIN_SKILL_MAPPINGS:
        ...
        for skill_name in sorted(os.listdir(abs_skills_dir)):
            ...
            if os.path.isdir(skill_path) and os.path.isfile(skill_md):
                ...
                # 1. Entry for skills/ directory level (e.g. .../plugins/adk-tools/skills/)
                skills_dir_entries.append({
                    "name": skill_name,
                    "files": sorted_files
                })

                # 2. Entry for plugin level (e.g. .../plugins/adk-tools/)
                plugin_dir_entries.append({
                    "name": f"skills/{skill_name}",
                    "files": sorted_files
                })

                # 3. Entry for single skill directory level (e.g. .../plugins/adk-tools/skills/adk-agent-architecture/)
        ...
        # Write skills/ directory level index.json
        ...
        # Write plugin directory level index.json
